...
::: {.callout-important}
## Caveat: this is not a universal law
Distance concentration is sharpest when the $p$ features are close to independent and identically distributed (the "random noise" setting simulated below). Real datasets are rarely like this — features are usually correlated, and the data often lie near a much lower-dimensional manifold embedded in the $p$-dimensional space (its **intrinsic dimensionality** is lower than $p$). In that case distances stay far more informative than this formula suggests. This is precisely *why* dimension reduction (PCA, t-SNE, etc.) works in practice: it recovers the low intrinsic dimensionality hiding inside a high nominal dimensionality.
:::
...
::: {.callout-important}
## Caveat: this applies to *local, non-parametric* methods
The exponential-sample-size argument above applies to methods that estimate quantities locally (KNN, kernel density/regression, histogram-based estimators), where every "cell" of the space needs its own observations. It is **not** a universal law for every statistical method. Regularized/structured models — Ridge and, especially, **Lasso regression** (Chapter 10) — are explicitly designed to work well even when $p > n$, by exploiting an assumption of *sparsity* (only a few predictors truly matter) rather than trying to fill every cell of the feature space with data. Keep this in mind when we reach Chapter 10: seeing Lasso perform well with many predictors and few observations is not a contradiction of the curse of dimensionality — it is a demonstration of how structural assumptions can partially escape it.
:::
...
::: {.callout-note}
## Common point of confusion: sign ambiguity
The **sign** of a loading vector $\mathbf{w}_k$ (and hence of its scores) is not identified: if $\mathbf{w}_k$ maximizes variance, so does $-\mathbf{w}_k$, and both are equally valid solutions. Different software, different R package versions, or even the same code re-run on slightly perturbed data can flip the sign of a component. This does **not** change the interpretation (a PC that separates "high scorers" from "low scorers" is the same finding whether high scorers get positive or negative scores) — but it does mean you should never be alarmed if your PC1 looks like the mirror image of a classmate's or a textbook's PC1.
:::
...
Parallel analysis is the most defensible criterion and is recommended for research purposes. Note that Kaiser's criterion was originally derived for **PCA** on a correlation matrix (where each standardized variable has variance 1); applying it to common **factor analysis** — which analyzes a *reduced* correlation matrix with communalities (not 1s) on the diagonal — is more contested in the literature and is well documented to often over-extract factors. Treat it as a rough starting point, not a rule to rely on alone.
...
3. Predictors are not severely multicollinear, and observations are independent — needed for $\mathbf{S}_W$ to be invertible and for the scatter-matrix estimates to be stable.
...
**Correlation filter:** Remove one variable from each highly correlated pair ($|r| > 0.85$; the exact threshold is a judgment call — common choices range from 0.80 to 0.90).
...
- `randomForest(formula, importance=TRUE)` computes permutation-based variable importance — a widely used approach for ranking features that may interact non-linearly (though not the only option; SHAP values and other permutation-importance variants are also common alternatives worth knowing about).
...
The **reconstruction error** measures how well the reduced representation can recreate the original data. Using the covariance definition from this chapter, $\mathbf{S} = \frac{1}{n-1}\mathbf{X}^T\mathbf{X}$, the *sum-of-squares* version of the reconstruction error is:

$$\text{SSE}(k) = \|\mathbf{X} - \hat{\mathbf{X}}_k\|_F^2 = (n-1)\sum_{j=k+1}^{p} \lambda_j$$

where $\hat{\mathbf{X}}_k = \mathbf{Z}_k \mathbf{W}_k^T$ is the rank-$k$ approximation. Dividing by $(n-1)$ gives the **mean reconstruction error per unit of total variance**:

$$\text{RE}(k) = \frac{\text{SSE}(k)}{n-1} = \sum_{j=k+1}^{p} \lambda_j$$

so RE$(k)$ (not SSE$(k)$) is what equals the sum of the discarded eigenvalues — confirming that PCA is the optimal linear reconstruction (Eckart–Young theorem). 

::: {.callout-warning}
## Watch the normalization when you compute this in R
It's easy to introduce an off-by-a-constant error here. `mean((X - X_hat)^2)` in R averages over **all $n \times p$ matrix entries**, i.e. it divides `SSE(k)` by $n p$, not by $(n-1)$. That is a *different, smaller* number than RE$(k)$ above by a factor of roughly $p$. If you want your computed reconstruction error to numerically match $\sum_{j=k+1}^p \lambda_j$, divide by $(n-1)$ explicitly — see the corrected R code below.
:::
...
Most criteria cluster around 3–4 components, but they do not all agree: parallel analysis and the elbow rule point to the more conservative $k=3$, while the 80% variance threshold and cross-validation point to $k=4$. There is no single "correct" answer here — it is a trade-off. Because the purpose in this scenario is a downstream classification model (where cross-validated predictive performance is the most directly relevant criterion), we choose $k=4$, accepting one extra dimension in exchange for the measured gain in predictive accuracy. Had the goal instead been a parsimonious 2D visualization, $k=3$ from parallel analysis would be the more defensible choice. The lesson: report *which* criteria you used and *why* you weighted one over another, rather than implying they all agreed.
...
recon_error <- sapply(1:ncol(mtcars), function(k) {
  ...
  # Divide by (n - 1), matching the S = X'X/(n-1) convention used
  # throughout this chapter -- NOT mean(), which divides by n*p and
  # would silently rescale the result by a factor of p.
  sum((X - X_hat)^2) / (n_obs - 1)
})
...
# Sanity check: RE(k) should equal the sum of the discarded eigenvalues,
# sum_{j=k+1}^{p} lambda_j (0 once k = p, i.e. nothing left to discard)
eigenvalues   <- pca_eval$sdev^2
discarded_sum <- sapply(1:ncol(mtcars), function(k) {
  if (k == ncol(mtcars)) 0 else sum(eigenvalues[(k+1):ncol(mtcars)])
})
stopifnot(all.equal(recon_error, discarded_sum, tolerance = 1e-8))
```
...
**Reconstruction Error** (with $\mathbf{S}=\mathbf{X}^T\mathbf{X}/(n-1)$):
$$\text{RE}(k) = \frac{\|\mathbf{X}-\hat{\mathbf{X}}_k\|_F^2}{n-1} = \sum_{j=k+1}^{p} \lambda_j \quad \left(\text{in R: } \texttt{sum(...)}/(n{-}1),\ \textbf{not}\ \texttt{mean(...)}\right)$$
