...
**A note on notation you'll need soon:** every distribution has a CDF
(`pnorm()`, `pt()`, ...) that goes from a value to a cumulative probability.
It also has an inverse — the **quantile function** (`qnorm()`, `qt()`, ...)
— that goes the other way: from a cumulative probability back to a value.
We write the quantile using a subscript equal to that cumulative
probability, e.g. $z_{0.975}$ or $t_{0.95,\,df}$. You'll use this constantly
in Chapter 3 to find critical values — we'll build it up properly there.
...
2. **Missing Data:** `Ozone` has 37 missing values (24.2%). How might this affect your probability calculations? What assumptions are you implicitly making when you use `na.omit()`?
