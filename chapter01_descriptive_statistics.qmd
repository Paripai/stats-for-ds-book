...
$$r_{XY} = \frac{S_{xy}}{\sqrt{S_{xx}\,S_{yy}}} = \frac{s_{XY}}{s_X \cdot s_Y} = \frac{\sum(x_i - \bar{x})(y_i - \bar{y})}{\sqrt{\sum(x_i-\bar{x})^2 \cdot \sum(y_i-\bar{y})^2}}$$

where the **sums of squares and cross-products** are

$$S_{xx}=\sum(x_i-\bar{x})^2, \quad S_{yy}=\sum(y_i-\bar{y})^2, \quad S_{xy}=\sum(x_i-\bar{x})(y_i-\bar{y}).$$

These connect to the covariance and standard deviations by $s_{XY}=S_{xy}/(n-1)$, $s_X=\sqrt{S_{xx}/(n-1)}$, and $s_Y=\sqrt{S_{yy}/(n-1)}$. The $1/(n-1)$ factors cancel, so all three forms give the same $r$ — the square root in the last form is simply *hidden* inside $s_X$ and $s_Y$.

**Computational form.** For hand calculation, the three sums can be obtained in a single pass through the data without first computing the means:

$$S_{xx}=\sum x_i^2-\frac{(\sum x_i)^2}{n}, \quad S_{yy}=\sum y_i^2-\frac{(\sum y_i)^2}{n}, \quad S_{xy}=\sum x_i y_i-\frac{(\sum x_i)(\sum y_i)}{n}.$$

Note $\sum x_i^2$ (square first, then sum) is **not** $(\sum x_i)^2$ (sum first, then square). This algebraic shortcut is convenient by hand but numerically unstable for large or highly correlated data — in software, always use `cor()` rather than coding this directly.
...
**Method 1 — deviation / covariance form:**
$$s_X = \sqrt{\frac{20}{3}} \approx 2.582, \quad s_Y = \sqrt{\frac{668.75}{3}} \approx 14.93$$
$$r = \frac{s_{XY}}{s_X \cdot s_Y} = \frac{115/3}{2.582 \times 14.93} \approx \frac{38.33}{38.55} \approx 0.994$$

**Method 2 — computational form** (five running sums, $n=4$):
$$\sum x_i = 20, \quad \sum y_i = 295, \quad \sum x_i^2 = 120, \quad \sum y_i^2 = 22425, \quad \sum x_i y_i = 1590$$
$$S_{xx} = 120 - \frac{20^2}{4} = 20, \quad S_{yy} = 22425 - \frac{295^2}{4} = 668.75, \quad S_{xy} = 1590 - \frac{20 \times 295}{4} = 115$$
$$r = \frac{S_{xy}}{\sqrt{S_{xx}\,S_{yy}}} = \frac{115}{\sqrt{20 \times 668.75}} = \frac{115}{115.65} \approx 0.994$$

Both routes give the same $r \approx 0.994$.
...
$$\bar{x} = \frac{1}{n}\sum x_i \qquad s^2 = \frac{1}{n-1}\sum(x_i-\bar{x})^2 \qquad \text{CV} = \frac{s}{\bar{x}} \times 100\% \qquad \text{IQR} = Q_3 - Q_1$$ $$r = \frac{S_{xy}}{\sqrt{S_{xx}\,S_{yy}}} = \frac{s_{XY}}{s_X s_Y} = \frac{\sum(x_i-\bar{x})(y_i-\bar{y})}{\sqrt{\sum(x_i-\bar{x})^2 \cdot \sum(y_i-\bar{y})^2}}$$
:::
