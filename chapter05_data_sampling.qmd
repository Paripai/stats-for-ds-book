...
$$E = z_{1-\alpha/2} \cdot \frac{\sigma}{\sqrt{n}} \quad \Rightarrow \quad n = \left(\frac{z_{1-\alpha/2} \cdot \sigma}{E}\right)^2$$

::: {.callout-warning}
## Watch the subscript here

This is exactly the convention trap flagged in Chapter 3: many sources
write $z_{\alpha/2}$ for this quantity, using the *right-tail* convention
(area $\alpha/2$ remaining above the value). Under **this book's**
left-tail-cumulative convention, that same value — e.g. 1.96 for a 95% CI
— is $z_{1-\alpha/2}$, not $z_{\alpha/2}$. Written the other way,
$z_{\alpha/2}$ would evaluate to $-1.96$ under our convention, the wrong
sign for a margin of error. Always check which convention a formula was
written in before plugging in `qnorm()`.
:::
...
where $d = |\mu_1 - \mu_2|/\sigma$ is Cohen's d. Note $z_{1-\beta}$ follows
the same convention: for 80% power ($\beta = 0.20$), $z_{1-\beta} =
z_{0.80} = \texttt{qnorm(0.80)} = 0.842$. In practice, use `pwr.t.test()`
as in Chapter 3.
