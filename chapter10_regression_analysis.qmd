...
The **Gauss-Markov theorem** guarantees that OLS is the Best Linear Unbiased Estimator (BLUE) when linearity, independent/uncorrelated errors with mean zero, and constant variance hold. Note that **normality is not required** for this result — it is needed only for exact t/F inference in small samples, not for OLS to be BLUE.
...
- Testing $H_0: \beta_1 = 0$ vs. $H_1: \beta_1 \neq 0$ (two-tailed), $n = 32$, $df = n - 2 = 30$:

**Method 1 — p-value.** $t_{\text{cal}} = -9.56$, $p < 0.001 < \alpha = 0.05$ $\Rightarrow$ **Reject** $H_0$.

**Method 2 — critical value.** Two-tailed test, so:
$$t_{\text{crit}} = \pm\, t_{0.975,\,30} = \pm\,\texttt{qt(0.975, 30)} = \pm 2.042$$
$$|t_{\text{cal}}| = 9.56 > 2.042 \quad \Rightarrow \quad \textbf{Reject } H_0$$

Both methods agree: the slope is highly significantly different from
zero ($t_{\text{cal}} = -9.56$, $df = 30$, $p < 0.001$ — reported as
$t(30) = -9.56$).
...
The F-distribution takes **two** degrees-of-freedom parameters, but the
same quantile convention from Chapter 3 applies: `qf(p, df1, df2)` returns
the value with cumulative probability $p$ to its left. The F-test is
always right-tailed (only large $F$ indicates evidence against $H_0$), so
the critical value at significance $\alpha$ is $F_{\text{crit}} =
...
- $\bar{R}^2 = 0.823$: The model explains 82.3% of variance after adjusting for the 3 predictors.

**Testing overall significance** ($H_0: \beta_1 = \beta_2 = \beta_3 = 0$), $n = 32$, $p = 3$ predictors, $df = (3,\, 28)$:

**Method 1 — p-value.** $F_{\text{cal}} = 48.96$, $p < 0.001 < \alpha = 0.05$ $\Rightarrow$ **Reject** $H_0$.

**Method 2 — critical value.** The F-test is always right-tailed:
$$F_{\text{crit}} = F_{0.95,\,(3,28)} = \texttt{qf(0.95, 3, 28)} = 2.947$$
$$F_{\text{cal}} = 48.96 > F_{\text{crit}} = 2.947 \quad \Rightarrow \quad \textbf{Reject } H_0$$

Both methods agree: at least one predictor has a non-zero coefficient
($F_{\text{cal}} = 48.96$, $df = (3, 28)$, $p < 0.001$ — reported as
$F(3, 28) = 48.96$).
...
- The model comparison table shows that adding `disp` and `cyl` raises both $R^2$ and adjusted $R^2$ slightly, but **raises AIC** relative to the 3-predictor model. This is a useful teaching moment: adjusted $R^2$ and AIC do not always agree, and AIC (or cross-validated error) is generally the more trustworthy guide against overfitting, since adjusted $R^2$ can still increase from predictors that add only marginal, noisy signal.
...
- **Cook's Distance:** Observation 117 has $D \approx 0.12 > 4/111 = 0.036$ — potentially influential
...
**Example 10.4.** Starting from 13 predictors in the `Boston` dataset, backward stepwise selection (AIC) retains 11 predictors, removing `indus` and `age`. In this dataset, every one of the remaining 11 predictors is significant at $p < 0.01$, so backward BIC selection converges to the **same 11-predictor model** — BIC's heavier penalty does not have any additional weak predictors left to prune here. (This is a useful illustration that AIC and BIC do not always disagree: with a small number of predictors, all clearly informative, both criteria can land on the same model.) All methods agree that `lstat` (lower status population) and `rm` (average rooms) are the most important predictors. *Run the code below and confirm these numbers against your own installed package versions before presenting them — small differences in variable coding or package version can shift borderline results.*
...
cv_mse <- function(model_formula, data, k = 10) {
  # Extract the outcome variable name from the formula so this
  # function works for ANY dataset/outcome (e.g. mpg in Exercise 10.4),
  # not just medv/Boston.
  outcome <- all.vars(model_formula)[1]
  folds   <- caret::createFolds(data[[outcome]], k = k)
  mse_vals <- sapply(folds, function(idx) {
    fit  <- lm(model_formula,
                data = data[-idx, ])
    pred <- predict(fit, data[idx, ])
    mean((data[[outcome]][idx] - pred)^2)
  })
  mean(mse_vals)
}
...
A degree-$d$ spline with $K$ knots has $d + K + 1$ degrees of freedom (the $+1$ comes from the intercept). For example, a cubic spline ($d=3$) with $K$ knots uses $4 + K$ degrees of freedom — consistent with the "4 knots" natural cubic spline used in Example 10.6 below.
...
- Linear model: $\bar{R}^2 \approx 0.038$ — poor fit; misses the curved relationship.
- Polynomial (degree 4): $\bar{R}^2 \approx 0.085$ — noticeably better but extrapolates poorly.
- Natural cubic spline (4 knots): $\bar{R}^2 \approx 0.086$ — similar fit, more stable.
- GAM with smooth: $\bar{R}^2 \approx 0.086$–$0.09$ — similar fit with automatic smoothness selection.

*(Figures above verified by refitting on the ISLR `Wage` dataset — confirm against your own knitted output, as the exact values depend on package/data version and whether `wage` or `logwage` is used as the outcome.)*
...
final_comparison <- data.frame(
  Model    = c("OLS (full)","Stepwise AIC",
                "Lasso","Ridge","GAM"),
  # NOTE: for the GAM, we use effective degrees of freedom (edf), not
  # raw coef() count. coef() on a mgcv::gam object returns every spline
  # basis coefficient, which vastly overstates model complexity since
  # penalization shrinks most of that basis flexibility away. edf is
  # the honest measure of how many "parameters worth" of flexibility
  # the fitted GAM is actually using.
...
- **Simple linear regression** — OLS estimation, slope interpretation, $R^2$, confidence vs. prediction intervals; the Gauss-Markov theorem justifies OLS as BLUE under Linearity, Independence, and Equal variance (**not** Normality — Normality is needed only for exact small-sample t/F inference).
